...
export const ParseHtml = (function () {
  ...
  function ParseState(opt_listType, opt_lineFormatting, opt_textFormatting) {
    ...
    this.lineFormatting = opt_lineFormatting || LineFormatting()
    this.textFormatting = opt_textFormatting || Formatting()
  }
  ...
  ParseOutput.prototype.newline = function (state) {
    ...
    this.lines.push(Line(this.currentLine, lineFormatting))
    this.currentLine = []
  }
  ...
  function parseNode(node, state, output) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      if (entity) {
        output.currentLine.push(
          new Text(
            sentinelConstants.ENTITY_SENTINEL_CHARACTER,
            new Formatting(entity.toAttributes())
          )
        )
        return
      }
    ...
    switch (node.nodeType) {
      case Node.TEXT_NODE:
        ...
        output.currentLine.push(Text(text, state.textFormatting))
        break
  ...
  function parseStyle(state, styleString) {
    ...
    for (var i = 0; i < styles.length; i++) {
      ...
      var prop = utils.trim(stylePieces[0]).toLowerCase()
      var val = utils.trim(stylePieces[1]).toLowerCase()
      switch (prop) {
        ...
        case "font-size":
          ...
          if (utils.stringEndsWith(val, allowedValues)) {
            size = val
          ...
          break
        case "font-family":
          var font = utils.trim(val.split(",")[0]) // get first font.
